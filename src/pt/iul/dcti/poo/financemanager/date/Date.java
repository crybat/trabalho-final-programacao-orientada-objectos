package pt.iul.dcti.poo.financemanager.date;
...
public class Date implements Comparable<Date> {
    ...
    private void checkInvariant() {
        ...
        if (getMonth().ordinal() < Month.JANUARY.ordinal()
                || getMonth().ordinal() > Month.DECEMBER.ordinal())
            throw new BadDate();
        if (getYear() < 0)
            throw new BadDate();
    ...
    public int diffInDays(Date d1) {
        ...
        int diffInDays = (int) Math.round(diffInMilis
                / (1000.0 * 3600.0 * 24.0));
        return diffInDays;
    }
